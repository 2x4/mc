...
// Change Log
// Sep.04, 2015
//  BioBisionAnimのスーパークラスをArrayListからHashMapに変更した．
// Jan.17, 1999
//  回転や座標をdoubleで表していたのをfloatに変更した．
// Dec.14
//  saveDataAsVRML97()を作成．VRML97形式の書き出しができるようにした．
//  BioVisionAnim.FrameDataのrotをVector3dからEulerに修正した．
...
//  private int framelength;
   ...
   * listに含まれるセグメントのフレームデータを，startindexから始まる
   * 位置にframelengthの長さに納まるように読み込む．
...
/*        
        Iterator<FrameData> f = segm.iterator();
        while (f.hasNext()) {
          BioVisionAnim.FrameData fd = (BioVisionAnim.FrameData) f.next();
          if (fd == null) {
            out.println("null");
          } else {
            out.println(fd.toString());
...
//        for (Iterator<FrameData> f = segm.iterator(); f.hasNext();) {
        for (FrameData fd : segm) {
//        BioVisionAnim.FrameData fd = (BioVisionAnim.FrameData) f.next();
          if (fd == null) {
            out.println("null");
          } else {
//            SFRotation sfr = new SFRotation(fd.rot);
//            out.print(sfr.toString() + ", ");
...
//	    segment.setDataSize(lastindex);
	...
	System.err.println("caught IOException at BioVisionAnim.Reader$run");
      }
      try {
	reader.close();
      } catch (IOException e) {
//	System.err.println("ファイル" + file.toString() + "をクローズできませんでした");
      }
    } // run()
   ...
   class Segment extends ArrayList<FrameData> {
    ...
    public final void setFrameTime(float ft) { this.frametime = ft; }
    public final void setParamUnit(int i, String unit) { this.unit[i] = unit; }
    public final void addFrameData(FrameData fd) { add(fd); }
    ...
    public final String getNameAsVRML() { return BioVisionAnim.rsegname.get(name); }
    ...
    public final float getFrameTime() { return frametime; }
    public final String getParamUnit(int i) { return unit[i]; }
    public final String[] getParamUnits() { return unit; }
...
/*      NumberFormat nf = NumberFormat.getInstance();
      ...
      return String.valueOf(nf.format(trans.x)) + " " +
              String.valueOf(nf.format(trans.y)) + " " +
              String.valueOf(nf.format(trans.z)) + " " +
              String.valueOf(nf.format(rot.getRollDeg())) + " " +
              String.valueOf(nf.format(rot.getPitchDeg())) + " " +
              String.valueOf(nf.format(rot.getYawDeg())) + " " +
              String.valueOf(nf.format(scale.x)) + " " +
              String.valueOf(nf.format(scale.y)) + " " +
              String.valueOf(nf.format(scale.z)); */
      return String.format("%4.4f %4.4f %4.4f %4.4f %4.4f %4.4f %4.4f %4.4f %4.4f", trans.x, trans.y, trans.z, rot.getRollDeg(), rot.getPitchDeg(), rot.getYawDeg(), scale.x, scale.y, scale.z);
    }
