    ...
    BioVisionHier exportBVH(File savefile) {
        ...
        for (i = 0; i < partslen; i++) {
            ...
            for (MotionCodePanel mcp : sp.getMotionCodeList()) {
                ...
                String strduration[] = mp7mgr.getSegmentDuration(motioncode).split("[TN]");
                int duration = Integer.parseInt(strduration[1]);
...
//      String linkedSegment[] = { "Body", "Body", "Hip", null };
    ...
    /**
     * ベースとなる部位の共有セグメントと接合する部位の共有セグメントの位置， 角度から，接合する部位の他のセグメントの位置，角度を決定する
     *
